# **M9 UD2 Lezione 4 - Derivata del reciproco e del quoziente**
...
> 📌 Un dettaglio della dimostrazione che all'orale viene chiesto: perché possiamo scrivere $\frac{1}{f(x+h)}$ per $h$ piccolo? Perché $f$, essendo derivabile, è continua in $x$ (UD1, Lezione 4), e una funzione continua con $f(x)\neq 0$ resta **separata da zero in tutto un intorno** (permanenza del segno, M08): quindi $f(x+h)\neq 0$ definitivamente e il rapporto incrementale di $\frac 1f$ è ben definito. Senza questo passaggio la dimostrazione avrebbe un buco.

> ⚠️ Non applicare mai la formula nei punti in cui $f(x_0)=0$: lì il reciproco non è nemmeno definito, quindi non ha senso chiedersi se è derivabile.
...
p'(x)\cdot\frac{1}{q(x)}
+
p(x)\cdot\frac{d}{dx}\left(\frac{1}{q(x)}\right)
$$
...
---
### **9. Errori frequenti sul quoziente**

- ❌ Invertire l'ordine al numeratore: la formula è $p'q - pq'$, **non** $pq'-p'q$; il segno sbagliato è l'errore più comune degli scritti.
- ❌ Dimenticare il **quadrato** al denominatore ($q$, non $q^2$).
- ❌ Applicare la formula dove $q(x_0)=0$ (la funzione non è definita lì).
- ❌ "Semplificare" la derivata del quoziente come quoziente delle derivate: $\left(\frac pq\right)'\neq\frac{p'}{q'}$ — controesempio: $\frac{x^2}{x}=x$ ha derivata $1$, mentre $\frac{(x^2)'}{(x)'}=2x$.
