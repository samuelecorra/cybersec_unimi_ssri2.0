# **M9 UD6 Lezione 2 - Problema di geometria solida (Maturità 2014)**
...
> 💡 Attenzione al dominio: qui è $(0,+\infty)$, aperto e illimitato, quindi **Weierstrass non si applica** e l'esistenza del minimo non è automatica. È lo studio del segno a garantirla: $S$ decresce su $(0,\sqrt[3]{5}]$ e cresce su $[\sqrt[3]{5},+\infty)$, quindi $\sqrt[3]{5}$ è minimo **globale** (criterio della derivata prima, UD5/L4, in versione globale). Coerentemente, agli estremi $S(x)\to+\infty$ sia per $x\to 0^+$ sia per $x\to+\infty$.
