...
Per essere **certi** di trovare almeno una collisione (cioè due messaggi diversi con lo stesso hash), basterebbe considerare $2^{|Z|}+1$ messaggi diversi — per semplice principio dei cassetti (_pigeonhole principle_). Infatti lo Z+1-esimo messaggio deve necessariamente collidere con uno dei precedenti.

Quindi, nel valutare una funzione hash, dovremo sicuramente tenere presente la dimensione del codominio $|Z|$.

Dovendo garantire una sicurezza invalicabile con semplice brute force, il numero di valori possibili deve essere sufficientemente grande. Il docente (2015) citava la soglia di $2^{80}$–$2^{160}$, osservando che $2^{40}$ non bastava già allora — GPU consumer dell'epoca erano in grado di calcolare $2^{40}$ hash in ore.

> ⚠️ **Contestualizzazione storica:** le videolezioni risalgono al 2015; i numeri citati vanno aggiornati agli standard attuali. $2^{80}$ di sicurezza (digest da 160 bit, cioè SHA-1) è oggi considerato **insufficiente**: la collisione pratica su SHA-1 è stata dimostrata nel 2017 (attacco SHAttered, Google). Il NIST raccomanda dal 2011 l'abbandono di SHA-1 e fissa a **128 bit il livello minimo di sicurezza**, corrispondente a un digest di almeno 256 bit (SHA-256, SHA-3-256). Il range moderno è quindi $2^{128}$–$2^{256}$, non $2^{80}$–$2^{160}$.
...
Infatti quando valutiamo la sicurezza di una hash function, definita come:

$$h : X → Z$$

dove la cardinalità del dominio è $|X| = m$ e la cardinalità del codominio è $|Z| = n$

$\rightarrow$ posso scegliere a caso diversi messaggi e...
$\rightarrow$ ...iniziare a verificare se ottengo due digest uguali.

A questo punto, la domanda è diventata **“Quanti messaggi devo generare per avere una probabilità di collisione ≥ 0.5 o comunque oltre una buona soglia ?”**

Prima di fornire una risposta, quali sono i parametri in gioco?

$n$ = numero di digest possibili (cioè $2^{|Z|}$)
$t$ = numero di messaggi generati a caso
$\varepsilon$ = probabilità di collisione

Questo attacco appena descritto è in relazione con un risultato statistico noto come **paradosso del compleanno**, che mostra come la probabilità di collisione cresca molto più rapidamente di quanto si possa intuire.
...
Per vedere la spiegazione integrale consulta la lezione di statistica: [M1/UD2/L6 – Legge del prodotto in generale e il paradosso del compleanno](../../../7_Statistica%20e%20Analisi%20dei%20Dati/M1_Probabilita_elementare/UD2/L6%20-%20Legge%20del%20prodotto%20in%20generale%20e%20il%20paradosso%20del%20compleanno.md)
