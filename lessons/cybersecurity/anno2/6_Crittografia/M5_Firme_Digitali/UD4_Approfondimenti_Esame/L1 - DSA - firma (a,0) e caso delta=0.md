...
> 📌 **Punto critico:** entrambe le fasi di verifica richiedono il calcolo di $\delta^{-1} \bmod q$. Questo inverso esiste **se e solo se** $\operatorname{mcd}(\delta, q) = 1$, cioè se $\delta \not\equiv 0 \pmod{q}$.
...
Se $\delta = 0$, l'inverso $\delta^{-1} \bmod q$ **non esiste** perché $\operatorname{mcd}(0, q) = q \ne 1$.
...
**1. La firma è invalida.** La verifica richiede $\delta^{-1} \bmod q$, che non esiste quando $\delta = 0$ (poiché $\operatorname{mcd}(0, q) = q \ne 1$). La firma $(a, 0)$ non può essere verificata da nessun destinatario.
