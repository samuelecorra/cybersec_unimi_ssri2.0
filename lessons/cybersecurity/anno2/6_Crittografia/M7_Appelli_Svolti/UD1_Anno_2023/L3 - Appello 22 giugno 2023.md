...
Cifrario affine: $E_{(a,b)}(x) = (a x + b) \bmod 26$, con $\operatorname{mcd}(a,26)=1$. Codifica A=0, …, Z=25. Dati: O=14, Z=25, T=19, I=8.
...
**Chiave:** $k = (a,b) = (7,5)$, con $\operatorname{mcd}(7,26)=1$ (valida). Verifica: $E(\text{O}) = 7\cdot14+5 = 103 \equiv 25 = \text{Z}$ ✓; $E(\text{T}) = 7\cdot19+5 = 138 \equiv 8 = \text{I}$ ✓.
...
Setup: $n = pq$ ($p,q$ primi distinti), $\varphi(n) = (p-1)(q-1)$, esponente pubblico $e$ con $\operatorname{mcd}(e,\varphi(n))=1$, esponente privato $d \equiv e^{-1} \pmod{\varphi(n)}$. Cifratura $c = m^e \bmod n$, decifratura $m' = c^d \bmod n$. Va dimostrato $m' = m$.
