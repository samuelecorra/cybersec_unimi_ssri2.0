...
Condizione necessaria e sufficiente: $\operatorname{mcd}(\det(K), 26) = 1$, ovvero il determinante di $K$ deve essere coprimo con 26.
...
Verifica: $\operatorname{mcd}(\det(K), 26) = \operatorname{mcd}(1, 26) = 1$ ✓
...
- **D2 (35 punti):** Feistel con funzione aritmetica mod 15 (diversa da tutte le versioni precedenti). La parte iii sulla sicurezza in funzione del valore numerico di $K$ è originale: analizzare $\operatorname{mcd}(2K, 15)$ per diversi valori di $K$ e discutere quando la funzione di round è biiettiva.
...
1. Verificare sempre $\operatorname{mcd}(\det(K), 26) = 1$ prima di procedere.
