...
Come calcolare velocemente il gcd per verificare la coprimalità? Si può usare l’**algoritmo di Euclide**. Ma in sede d'esame si può anche dare per garantito che il prof abbia scelto $p$ e $q$ tali che $e$ sia coprimo con $\varphi(n)$ per "costruzione". In poche parole computiamo e basta.
