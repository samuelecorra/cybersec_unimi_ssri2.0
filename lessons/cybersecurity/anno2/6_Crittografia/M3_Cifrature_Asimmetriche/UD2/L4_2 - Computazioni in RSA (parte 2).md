...
> 💡 **Perché ci serve e da dove viene.** Per RSA dobbiamo calcolare $d = e^{-1} \bmod \varphi(n)$, cioè trovare un intero $d$ tale che $e \cdot d \equiv 1 \pmod{\varphi(n)}$. L’algoritmo di Euclide classico calcola solo il $\operatorname{mcd}$; la versione *estesa* fa di più: trova anche i due interi $x$ e $y$ della cosiddetta **identità di Bézout**, cioè esprime il $\operatorname{mcd}$ come combinazione lineare intera di $a$ e $n$. Quando il $\operatorname{mcd}$ vale $1$, il coefficiente $x$ è esattamente l’inverso che cerchiamo.

L’**algoritmo esteso di Euclide** permette di:

- Calcolare il **$\operatorname{mcd}(a, n)$**
    
- Trovare **l’inverso moltiplicativo** di $a$ modulo $n$, se $\operatorname{mcd}(a,n)=1$
    

Restituisce tre valori $(d, x, y)$ tali che:

$$  
d = \operatorname{mcd}(a, n) = a \cdot x + n \cdot y  
$$

> 📌 **Identità di Bézout.** Questa scrittura $d = ax + ny$ si chiama identità di Bézout e vale sempre: il $\operatorname{mcd}$ di due interi si può sempre esprimere come loro combinazione lineare (con coefficienti interi, anche negativi). Riducendo entrambi i lati modulo $n$: $d \equiv ax \pmod{n}$. Se $d = 1$, allora $1 \equiv ax \pmod{n}$, cioè $x$ è proprio l’inverso di $a$ modulo $n$.
...
> 📌 **Come funziona il ritorno indietro.** La chiamata ricorsiva scende esattamente come l’Euclide classico (riducendo $a \bmod n$ ad ogni passo), finché $n = 0$: a quel punto il $\operatorname{mcd}$ è $a$ stesso, e i coefficienti banali sono $x=1, y=0$ (perché $a \cdot 1 + 0 \cdot 0 = a$). Poi, risalendo, ogni livello *ricostruisce* i propri $x$ e $y$ da quelli del livello sottostante tramite la formula $x = y’$, $y = x’ - \lfloor a/n \rfloor \cdot y’$. Questa formula deriva dalla sostituzione dell’identità $a = \lfloor a/n \rfloor \cdot n + (a \bmod n)$ nell’identità di Bézout del livello inferiore.
...
$$
\operatorname{mcd}(99, 78) = 3, \qquad 99 \cdot (-11) + 78 \cdot 14 = -1089 + 1092 = 3
$$
...
> 💡 **Raccordo con RSA.** Questo è esattamente il caso che ci serve: vogliamo trovare $d$ tale che $e \cdot d \equiv 1 \pmod{\varphi(n)}$. La condizione di esistenza diventa $\operatorname{mcd}(e, \varphi(n)) = 1$, e questa è precisamente la condizione che verifichiamo nella generazione delle chiavi RSA (passo 5 della sezione 9). Se non è soddisfatta, si cambia $e$ e si riprova.

La congruenza $ax \equiv 1 \pmod{n}$ ha **soluzione unica** modulo $n$ se e solo se $\operatorname{mcd}(a, n) = 1$.
...
> 💡 $8 = 2^3$ non è primo: gli unici numeri in $[1,7]$ coprimi con $8$ sono quelli **dispari** (i pari condividono il fattore $2$ con $8$, quindi $\operatorname{mcd} > 1$ e l'inverso non esiste).

| $a$ | $\operatorname{mcd}(a,8)$ | $a^{-1} \pmod{8}$ |
|---|---|---|
| 1 | 1 | 1 |
| 2 | 2 | — (non esiste) |
| 3 | 1 | 3 |
...
> 💡 $7$ è primo: per il piccolo teorema di Fermat, ogni $a \in [1, 6]$ soddisfa $\operatorname{mcd}(a,7)=1$, quindi tutti hanno inverso. In un campo $\mathbb{Z}_p$ con $p$ primo l'inverso esiste sempre per ogni elemento non nullo.
...
Siano $m_1, m_2, \dots, m_t$ interi positivi e **a coppie coprimi** ($\operatorname{mcd}(m_i, m_j)=1$ per $i \ne j$).
...
5. **Controlla** $\operatorname{mcd}(e,\, \varphi(n)) = 1$ tramite Euclide classico.
...
- La **congruenza lineare** $ex \equiv 1 \pmod{\varphi(n)}$ ha soluzione unica se e solo se $\operatorname{mcd}(e, \varphi(n)) = 1$, condizione verificata a ogni tentativo di scelta di $e$.
