...
dove i parametri sono: transazione `T`, oggetto `x`, codice di errore `errcode` e tempo massimo di attesa `timeout`.

`errcode` di solito è $0$ se la richiesta è stata soddisfatta, oppure un **codice di errore** che indica il motivo del rifiuto.
...
![](imgs/Pasted%20image%2020260824003734.png)
