...
Quando la condizione riguarda solo una delle due espressioni, la selezione può essere **anticipata** rispetto al join.
Qui mostriamo il caso in cui la condizione riguarda **solo $E_2$**:
...
- $X_1 = \{\text{Matr, Cognome, Nome}\}$ → attributi di `STUDENTI`
- $X_2 = \{\text{Matr, Corso, Voto\}}$ → attributi di `ESAMI`
- $X_1 \cap X_2 = \{\text{Matr}\}$ → attributo comune necessario per il join
- $Y_2 = \{\text{Matr, Corso}\}$
...
Y_2 = \{\text{Matr,Corso}\}
\supseteq
X_1 \cap X_2 = \{\text{Matr}\}
$$

Possiamo quindi eliminare **prima del join** da `ESAMI` tutti gli attributi che non servono né nel risultato finale né per effettuare il join.
