...
- **Minimo assoluto**:
  ...
  $$
  $$
  \text{val} - \pi_B(\text{val}\bowtie_{B > B'}(\rho_{B' \leftarrow B}(\text{val})))
  $$
- **Massimo assoluto**:
  $$
  \text{val} := \pi_B(r)
...
- **Minimo relativo (per gruppo):**
  $$
  r - \pi_{A,B}(r\bowtie_{A=A' \land B > B'}(\rho_{A',B' \leftarrow A,B}(r)))
  $$
...
- **Rispondente a minimo:**
  $$
  \pi_A(r - \pi_{A,B}(r\bowtie_{B > B'}(\rho_{A',B' \leftarrow A,B}(r))))
  $$
...
$$
\pi_M(\text{VOTI} - \pi_{M,V}(\text{VOTI}\bowtie_{V > V'}(\rho_{M',V' \leftarrow M,V}(\text{VOTI}))))
$$
...
$$
\pi_C(\text{ESAMI} - \pi_{C,D}(\text{ESAMI}\bowtie_{D < D'}(\rho_{C',D' \leftarrow C,D}(\text{ESAMI}))))
$$
