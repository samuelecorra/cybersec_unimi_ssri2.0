# **M3 UD2 Lezione 2 - HeapSort e SelectionSort**
...
#### **Complessità di `restauraheap`**

- E' una funzione ricorsiva, basata sulla discesa di un nodo dalla radice fino alla foglia, se prendiamo ovviamente in considerazione il caso peggiore, cioè quando il nodo da restaurare è la radice e deve scendere fino alla foglia più profonda, allora la complessità di `restauraheap` è proporzionale all'altezza dell'albero, che per un heap completo è $O(\log n)$.

- Un modo equivalente di vedere la complessità è $O(h)$, dove $h$ è l'altezza dell'albero, e per un heap completo, l'altezza è approssimativamente $\log_2 n$, quindi i conti tornano perfettamente.

Come stabiliamo l'altezza di un nodo, dal momento che finora parlavamo di profondità?

- foglia → profondità 0;
- nodo non foglia → profondità 1 + profondità del padre oppure max_altezza_figli + 1.
