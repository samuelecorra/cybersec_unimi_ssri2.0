...
#### **Fase 0 — Apertura e triage (stessa cattura del 02/07)**

La traccia si scarica in `pcap` o `pcapng` — per Wireshark è indifferente (`File → Open` o drag & drop). `Statistics → Capture File Properties` riporta **1.790 pacchetti in ~14 minuti e mezzo**; `Statistics → Protocol Hierarchy` mostra **TCP al 92% (1.651 pacchetti)** — ⚠️ la riga *HTTP* ne conta solo **6** perché la gerarchia attribuisce ogni pacchetto al livello più alto decodificato: i segmenti dei download e gli ACK restano alla riga *TCP*, e la riga *Data* (171) è il payload della **porta non standard 2032** (da annotare subito). I volumi per porta si leggono col filtro: `tcp.port == 80` → 1.389 pacchetti, `tcp.port == 2032` → 262 (voce *Displayed* nella barra di stato); il resto è rumore NBNS/LLMNR/DHCP. `Statistics → Conversations` rivela un **solo host locale**, `10.6.27.102`, verso pochi IP esterni. Filtri chiave: `tcp.stream eq 0` (prima connessione), `http.request` (tutti i `GET` → i file richiesti), `File → Export Objects → HTTP` (⚠️ estrae **malware reale**: leggere solo nome e `Content-Type`, non aprire), `dns` (risoluzioni) e `tcp.port == 2032` (beacon C2). Il percorso di triage completo, con la mappa domanda → filtro, è nella **Fase 0** della [soluzione del 02/07/2025](../lab_2_luglio_2025/Soluzione_Lab_02_07_2025.md).

- **Prima connessione TCP:** `10.6.27.102:49157 → 23.63.254.163:80` (a `t ≈ 14,2 s`). Client `10.6.27.102`, MAC `00:08:02:1c:47:ae` (**HP**); MAC di destinazione = gateway `20:e5:2a:b6:93:f1` (**NETGEAR**), perché il server è remoto.
...
- **Descrizione della sessione:** catena di infezione — controllo NCSI di Windows → DNS di `smart-fax.com` → download del finto documento-fattura → download di `knr.exe` → connessioni ripetute verso **`dunlop.hopto.org` (`23.105.131.229`) sulla porta 2032**, canale di **comando e controllo (C2)** del malware. ⚠️ Nessun redirect da `msftncsi.com`: gli eventi sono legati solo dalla cronologia (innesco fuori cattura, verosimilmente un link in una email). 💡 Il `GET` del `.doc` ha lo User-Agent di IE11, quello di `knr.exe` la stringa `Mozilla/4.0 (compatible; MSIE 7.0; … .NET CLR …)` dei componenti WinInet/URLMon: il payload l'ha scaricato **il documento aperto**, non l'utente.
...
![](imgs/Pasted%20image%2020260708202034.png)
